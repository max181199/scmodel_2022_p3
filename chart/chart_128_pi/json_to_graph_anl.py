...
for t in range(0, k + 1):
    if (t == 0 or t == 20 or t == k): 
        ...
        ax.set_title('Lx=Ly=Lz=3.14, N=' + str(size) + ', T=1, K=' + str(220) + ', t=' + str(t) + ', s=' + str(step), fontsize = 120)
        ...
        data = json.load(f)
        transform = interp1d([-1, 1],[0, 100])
        x = list(map(lambda x: x['i'], data['data']))
        y = list(map(lambda x: x['j'], data['data']))
        z = list(map(lambda x: x['k'], data['data']))
        c = list(map(lambda x: float(transform(x['anl'])), data['data']))
        s = list(map(lambda x: 900, data['data']))
        ...
        f.close()
        plt.savefig('anl_' + str(size) + '_' +  str(step) + '_' + str(t) + '_' + str(g) + '.png')
        fig.clear()
